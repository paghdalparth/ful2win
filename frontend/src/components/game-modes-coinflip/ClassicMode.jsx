...
export default function ClassicMode({ onBack }) {
  ...
  const handlePlay = (entry, winAmount) => {
    ...
    const deducted = deductMoney(entryFee, `CoinFlip Classic Mode Entry Fee (₹${entryFee})`);
    ...
    if (deducted) {
      ...
      navigate(`/games/coinflip/play/classic?price=${entryFee}`);
    }
  };

  return (
    <GameModeLayout title="CoinFlip" onBack={onBack}>
