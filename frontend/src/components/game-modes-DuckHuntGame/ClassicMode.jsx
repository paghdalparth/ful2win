...
export default function ClassicMode({ onBack, gameTitle = "DuckHuntGame" }) {
  ...
  const handlePlay = (entry, winAmount) => {
    ...
    const deducted = deductMoney(entryFee, `DuckHuntGame Classic Mode Entry Fee (₹${entryFee})`);
    ...
    if (deducted) {
      ...
      localStorage.setItem('currentGameInfo', JSON.stringify({
        ...
        gameType: 'DuckHuntGame',
        mode: 'Classic'
      }));
      
      // Navigate to the game lobby with the proper parameters
      navigate(`/games/DuckHuntGame/play/classic?price=${entryFee}`);
    }
  };

   return (<div className="h-screen bg-cover bg-center" style={{ backgroundImage: "url('/DuckHuntGame.jpg')" }}>
